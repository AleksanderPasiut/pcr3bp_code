...
public:
    ...
    void set_param(const std::vector<Aquila::ParamPacket<double>>& packet_vector)
    {
        ...
        param.x0 = this->get_param(0);
        param.y0 = this->get_param(1);
        param.px0 = this->get_param(2);
        param.py0 = this->get_param(3);
        param.t = this->get_param(4);
        param.point_count = this->get_param(5);
...
int main(int argc, char* argv[])
{
    ...
    Serpent::SglHostWindow<Core> window(
        Ursa::create_window_properties("std evolution raw"), argc, argv);
